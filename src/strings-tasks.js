...
function isPalindrome(str) {
  const editStr = str.toLowerCase().replace(/[^a-zA-Z]/g, '');
  const reversedStr = editStr.split('').reverse().join('');

  return editStr === reversedStr;
}
...
function reverseWords(str) {
  ...
  const sortedWords = words.map((world) => {
